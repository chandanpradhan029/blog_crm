            ...
            if (in_array($FileExt, ['png', 'jpg', 'bmp', 'tiff'])) {
                if ($FileSize <= 5242880) { // 5MB limit for other file sizes
                    ...
                    if (move_uploaded_file($FileTmp, $Dstn)) {
                        ...
                        $ImagePath = 'http://' . $_SERVER["HTTP_HOST"] . '/Admin/' . $Dstn;

                        $Query = "INSERT INTO blog_post (Post_Tag, Post_Title, Post_Feature_Image, Post_Content, Posted_By, Post_Date, Post_Slug) 
                                  VALUES ('$PostTag', '$PostTitle', '$ImagePath', '$PostContent', '" . $_COOKIE['LoggedInID'] . "', '$cust_date', '$PostSlug')";
